...
class _ForumsState extends State<Forums> {
  ...
  _loadMoreData(int loaderPage) {
    setState(() {
      getForumApiV2(loaderPage, '', currentTabBreed);
  ...
  Future getForumApiV2(int page, String search, int type) async {
    var res = await AllApi.getMethodApi(
        "${ApiStrings.forums}?type=$type&page=$page&limit=20&search=$search");
    var result = jsonDecode(res.toString());
    if (result['status'] == 200) {
      ...
      _listForums.addAll(_allForumsResponseModel.data!);
    } else {
      toaster(context, result['message'].toString());
    }
    setState(() {
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return GestureDetector(
                  onTap: () async {
                    Map<String, dynamic> mapping = {
                      'image': _listForums[index].category
                          ?.icon !=
                          null &&
                          _listForums[index].category!.icon
                              .toString()
                              .isNotEmpty
                    && _listForums![index].category!.icon!.contains('http')
                          ? '${_listForums[index].category!.icon}'
                          : '',
                      'desc': sharedPref
                          .getString(
                          ...
                          .toString() ==
                          'en'
                          ? _listForums[index].description
                          .toString()
                          : _listForums[index].descriptionFr
                          .toString(),
                      'topic': _listForums[index].title
                          .toString(),
                      'forumId': _listForums[index].id,
                    };
                    // Map<String, dynamic> mapping = {
                    //   'image': getForum.data![index].dogBreed
                    //       ?.icon !=
                          ...
                          child: Row(
                            ...
                            children: [
                              ClipRRect(
                                borderRadius: BorderRadius.all(
                                    Radius.circular(50)),
                                child: _listForums[index].category?.icon != null &&
                                    _listForums[index]
                                    .category!
                                        .icon
                                        .toString()
                      ...
                      && _listForums[index].category!.icon!.contains('http')
                              ...
                              Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    MyString.bold(
                                        _listForums[index].category!.name
                                            .toString(),
                                        14,
                                        Color(0xff4F2020),
                                        TextAlign.start),
  ...
  Future getData() async {
    ...
    await getForumApiV2(page, '', currentTabBreed);
    // await getForumQuestionApi(page, '');
    // await getReplyTopic(page, '');
    setState(() {
